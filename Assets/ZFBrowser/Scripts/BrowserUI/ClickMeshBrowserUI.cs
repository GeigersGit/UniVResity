	 ...
	 */
		...
		protected virtual Ray LookRay {
			get { 
				...
				if (mouseMode || !pointer) {
					...
					//Vector3 direction = (pointer.transform.position - Camera.main.transform.position);
					//Vector3 direction = -Vector3.forward;
					Vector3 direction = Vector3.forward;
					Ray ray = new Ray (pointer.transform.position, direction);
					//Debug.DrawRay(pointer.transform.position, direction, Color.red, 0.5f);
					return ray;
				}
			}
		}
		...
		public virtual void InputUpdate() {
			...
			if (mouseMode || !pointer) {
				...
				if ((pointer.transform.position - hit.point).magnitude < 0.02f && (pointer.transform.position - hit.point).magnitude > 0f) {
